...
export default function TvSeriesPage({ setIsAuthenticated }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % yourwatchlist.length);
    }, 5000);
    return () => clearInterval(interval);
  }, []);

  const filteredYourWatchList = yourwatchlist.filter(yourwatchlists => 
    (selectedGenre === "All" || yourwatchlists.genre === selectedGenre) && 
    yourwatchlists.title.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-black text-white font-sans flex flex-col justify-between">
      ...
      <div className="flex-grow mt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6">
          {filteredYourWatchList.map((yourwatchlists, index) => (
            <div key={index} className="bg-gray-900 p-4 rounded-lg shadow-lg border border-gray-700">
              <img src={yourwatchlists.image} alt={yourwatchlists.title} className="w-full h-60 object-cover rounded-lg" />
              <div className="mt-4">
                <span className="bg-gray-700 px-2 py-1 text-xs rounded font-medium">{yourwatchlists.genre}</span>
                <h4 className="text-lg font-semibold mt-2 leading-tight">{yourwatchlists.title}</h4>
                <p className="text-gray-400 text-sm mt-1">{yourwatchlists.description}</p>
              </div>
            </div>
          ))}
        </div>
